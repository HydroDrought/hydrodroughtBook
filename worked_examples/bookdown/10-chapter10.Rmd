...
guadiana_period <- guadiana %>%
	filter(time >= as.Date("1960-01-01") & time <= as.Date("2000-12-31"))
...
influenced <- guadiana_period %>%
  dplyr::select(time, discharge = Qobs) %>%
  mutate(
    year = water_year(time)
  ) %>%print()
```
...
threshold <- benchmark %>%
  ...
  # double check using the correct period
...
p <- ggplot(benchmark, aes(x=jdate)) + 
	...
	geom_line(data = ya, aes(y=threshold), colour="red") + 

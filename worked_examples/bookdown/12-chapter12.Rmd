...
## Worked example 12.1: Classifying an impact report by the EDII impact category and type scheme
...
## Worked example 12.2: Drought Impact Model
...
fit_data <- spi_impacts %>%
    dplyr::select(date, year, month, index, index_val, impact) %>%
...
predict_df <- fit_data %>%
    dplyr::mutate(predict = plogis(predict_ts$fit)) %>%
    dplyr::mutate(predict_upper = plogis(predict_ts$fit + (qnorm(0.025) * predict_ts$se.fit))) %>% 
    dplyr::mutate(predict_lower = plogis(predict_ts$fit - (qnorm(0.025) * predict_ts$se.fit))) %>%
    dplyr::select(-spi_3, -spei_12, -spi_12) %>%
	print()
```

...
ngaruroro <- ngaruroro %>%
  ...
  mutate(
    year = group_ts(time, by = "year", start = "-09-01")
  ) %>%
  print()

incomplete <- ngaruroro %>%
  filter(!is.na(discharge)) %>%
  count(year, name = "days.with.data") %>%
  mutate(
    days.in.year = days_in_year(year, origin = "-09-01"),
    days.missing = days.in.year - days.with.data
  ) %>%
...
```
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.cap=fig.cap}
thres <- droughts %>%
...
d <- droughts %>%
  mutate(
    year = water_year(first.day, start = "-09-01"),
...
pooled %>%
  mutate(
    year = group_ts(first.day, by = "year", start = "-09-01")
  ) %>%
  group_by(year) %>%
  summarise(
    n.droughts = n(),
...
```
```{r, echo=TRUE, message=FALSE, warning=FALSE, fig.cap=fig.cap}

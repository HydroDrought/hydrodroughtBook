---
title: "Low flow frequency analysis (dataset from first edition)"
...
^[Tobias: Right now the old dataset is used to make sure we can reporoduce the numbers from the first edition. Changing to the new dataset will change the results quite a little.]
...
ngaruroro.n <- intl %>%
  ...
  filter(year %in% 1964:1999)

# am.dig <- tibble(
#   year = c(1965L, 1966L, 1967L, 1969L, 1970L, 1971L, 
#            1972L, 1973L, 1974L, 1975L, 1976L, 1977L, 1978L, 1980L, 1981L, 
#            1982L, 1983L, 1984L, 1985L, 1986L, 1989L, 1990L, 1991L, 1992L, 
#            1993L, 1994L, 1995L, 1996L, 1997L, 1998L, 1999L, 2000L), 
#   am = c(4.792, 
#          4.827, 3.23, 3.837, 3.859, 4.712, 3.863, 2.788, 3.023, 4.675, 
#          4.85, 4.101, 2.582, 6.484, 5.466, 3.43, 2.703, 4.867, 4.07, 3.983, 
#          3.895, 4.041, 3.977, 4.558, 3.814, 3.256, 4.483, 5.669, 3.953, 
#          3.372, 4.575, 3.837)
# )
...
ng <- read_rds("~/Documents/boku/hydroDroughtDev/data/firstEdition/global.rds") %>%
  arrange(River) %>%
  filter(River == "Ngaruroro") %>%
  select(discharge) %>%
  unnest(discharge) %>%

...
To calculate a (riverflow) quantiles the function `lfquantile()`^[Tobias: I've written a small wrapper function around `base::quantile()` to circumvent the naming problem and the transformation into non-exceedande frequencies.] can also be used directly. The exact (interpolated) values for $Q_{95}$, $Q_{90}$ and $Q_{80}$ would be: 

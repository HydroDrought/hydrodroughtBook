...
In this example we are going to use river flow data from the river Ngaruroro which is part of the `international` dataset in the package **hydroDrought**. This dataset becomes accessible by loading the **hydroDrougt** package. ^[Tobias: Currently the variable in the dataset is called *discharge* but we should consider using *flow* or *riverflow* for consistency. *river flow* (which a space in it) is not advisable as a variable name.]
...
exceedance_frequency <- function(flow)
{
  ...
  # largest rank (= number of non-missing values)
...
- Corresponding values of riverflow ($Q$ in $m^3s^{-1}$) and exceedance frequency ($EF_{Q_i}$ in %) are tabulated^[Tobias: Tabulation was done implicitely in the sections before. ].

- The two columns are sorted by $EF_{Q_i}$^[Tobias: Sorting is not required. I suggest to delete the section "Tabulation of the FDC."].

# Plot of the FDC
The sorted table columns are then plotted (Figure 5.2). The ordinate axis is here logarithmic^[Tobias: I suggest to label the y-axis with "Flow percentile"]. 
...
Alternatively, in case of large differences between successive values, a linear interpolation can be used. To calculate a (riverflow) quantile the R function `quantile` can also be used directly. The exact (interpolated) value for $Q_{90}$ would be: 
